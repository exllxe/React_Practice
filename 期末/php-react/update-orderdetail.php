...
$search_pid = mysqli_query($db_connection, "SELECT * FROM `product` WHERE product.ProdID = '$prodid'");

if (mysqli_num_rows($search_pid) > 0) {

    $search_orderid = mysqli_query($db_connection, "SELECT * FROM `salesorder` WHERE salesorder.OrderId = '$orderid'");
    if(mysqli_num_rows($search_orderid) > 0){
